# Deployment (Render) -- reference design, not what's running today

**This is not the current production setup.** Production actually runs on a
home Fedora server behind Tailscale Funnel, deployed by hand -- see
[DEPLOYMENT.md](DEPLOYMENT.md). This file is kept as the original,
fully-worked-out design for a managed-host deployment (Render), in case the
app ever migrates there. Don't follow this file to operate current production.
